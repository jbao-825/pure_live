...
class AppPathManager {
  ...
  /// lifetime, and Windows releases the lock when the owning process ends (a
  /// hard kill included). An *acquirable* lease therefore proves that the data
  /// root next to it belongs to a window that is no longer running.
  static const String instanceLeaseFileName = '.instance.lease';

  /// Instance ids generated by [WindowsMultiInstanceLauncher.launch]. Only
  /// directories matching this shape are ever candidates for removal.
  static final RegExp _instanceDirectoryName = RegExp(r'^window_\d+_\d+$');

  /// Leases held by this process, kept referenced so the handle - and with it
  /// the lock - outlives startup for as long as the window lives.
  static final List<RandomAccessFile> _heldInstanceLeases = [];

  /// A window that was just spawned has not taken its lease yet, so a young
  /// directory is never treated as abandoned.
  static const Duration _reclaimMinIdle = Duration(minutes: 5);

  /// Reclaiming is spread over several launches so a long-neglected
  /// installation drains gradually instead of stalling a single startup.
  static const int _reclaimMaxPerRun = 32;
  ...
  Future<void> initialize({String instanceId = ''}) async {
    ...
    if (Platform.isWindows) {
      if (sanitizedInstanceId.isNotEmpty) {
        await _holdInstanceLease(rootPath);
      }
      // Fire-and-forget: reclaiming must not delay startup, and it only ever
      // touches directories proven to be abandoned.
      unawaited(
        _reclaimAbandonedInstances(
          parentRoot: sanitizedInstanceId.isEmpty ? rootPath : p.dirname(rootPath),
          ownInstanceId: sanitizedInstanceId,
        ),
      );
    }
  ...
  Future<void> _holdInstanceLease(String rootPath) async {
    try {
      final handle = await File(p.join(rootPath, instanceLeaseFileName)).open(mode: FileMode.append);
      await handle.lock(FileLock.exclusive);
      _heldInstanceLeases.add(handle);
    } catch (error) {
      // Startup must never depend on the lease. Losing it only makes this
      // directory look reclaimable, and the age guard below still covers the
      // window that is currently starting up.
      log('实例租约获取失败 ($rootPath): $error');
    }
  }

  /// Removes the data roots of child windows that are no longer running.
  ///
  ...
  Future<void> _reclaimAbandonedInstances({
    ...
    try {
      ...
      var reclaimed = 0;
      await for (final entity in parent.list(followLinks: false)) {
        if (reclaimed >= _reclaimMaxPerRun) return;
        if (entity is! Directory) continue;

        final name = p.basename(entity.path);
        if (name == ownInstanceId) continue;
        if (!_instanceDirectoryName.hasMatch(name)) continue;
        if (DateTime.now().difference((await entity.stat()).modified) < _reclaimMinIdle) continue;
        if (!await _leaseIsFree(entity)) continue;

        try {
          await entity.delete(recursive: true);
          reclaimed++;
        } catch (error) {
          // Antivirus and dying processes can hold transient handles. A failed
          // removal is retried on a later launch.
          log('实例数据目录清理失败 (${entity.path}): $error');
        }
      }
    } catch (error) {
      log('实例数据目录扫描失败 ($parentRoot): $error');
    }
  }

  /// Whether no live process still holds the lease of [instanceDirectory].
  Future<bool> _leaseIsFree(Directory instanceDirectory) async {
    RandomAccessFile? handle;
    try {
      handle = await File(p.join(instanceDirectory.path, instanceLeaseFileName)).open(mode: FileMode.append);
      await handle.lock(FileLock.exclusive);
      return true;
    } catch (_) {
      // Held by a running window, or the directory is not writable.
      return false;
    } finally {
      try {
        await handle?.close();
      } catch (_) {
        // A probe handle that refuses to close changes nothing here.
      }
    }
  }
